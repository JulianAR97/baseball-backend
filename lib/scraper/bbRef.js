...
import { compareNames } from '../helpers.js'
...
export const scrape40Man = async (teamID, year = new Date().getFullYear()) => {
  ...
  if (table[0]) {
    ...
    players.sort((a, b) => compareNames(a.name, b.name))
    players.forEach(p => p.country = p.country.toUpperCase())
    console.log('done')
    return players
  }
...
// export const scrapeFullSeasonRoster = async (teamID, year = new Date().getFullYear()) => {
//   const url = `https://www.baseball-reference.com/teams/${teamID}/${year}.shtml`
//   let table = await getData(url, '#appearances')
...
//   const url = 'https://baseball-reference.com/'
//   console.log('Scraping team names...')
//   let data = await getData(url, "[data-stat=team_ID]")
//   data = data.toArray()
//   let teamIDs = depthFirstData(data)
//     .filter((v, i, a) => a.indexOf(v) === i) // unique array
//     .filter(t => t.length === 3) // gets rid of 2 unwanted elements : 'AL' and 'NL'
...
//   // const url = `https://www.baseball-reference.com/teams/${teamID}/${year}.shtml`
//   const newUrl = `https://www.baseball-reference.com/teams/LAA/2021.shtml`

//   let data = await getData(newUrl, "table")
//   data = data.toArray()

//   for (let team of teams) {
//     let year = new Date().getFullYear()
//     while (true) {
//       let roster = await scrape40Man(teams)
//     }
//     let roster = await scrape40Man(team, year)
