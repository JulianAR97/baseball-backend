...
export const scrapeTeamData = async (teamID) => {
  ...
  return {teamID, name, logo}
}

export const scrapeRecords = async () => {
  let records = {}
  ...
  const $ = cheerio.load(content)
  const alTable = $('#standings_AL').toArray()[0]
  const nlTable = $('#standings_NL').toArray()[0]

  const alData = depthFirstData(alTable.children.find(c => c.name === 'tbody').children)
  const nlData = depthFirstData(nlTable.children.find(c => c.name === 'tbody').children)
  // add AL and NL to beginning of arrays so that we can iterate over a concatenated array dynamically
  alData.unshift('AL')
  nlData.unshift('NL')
  
  let data = alData.concat(nlData)
  
  // We just need to keep track of the current league and current division
  // so we can add team records to respective league and division
  let currentLge = ''
  let currentDiv = ''
  for (let i = 0; i < data.length; i++) {
    let ele = data[i];

    if (ele.match(/^[A-Z]{2}$/)) currentLge = ele
    if (ele.match(/^[A-Z].*[a-z]$/)) currentDiv = ele
    if (!records[currentLge] && !!currentLge) records[currentLge] = {}
    if (!records[currentLge][currentDiv] && !!currentDiv) records[currentLge][currentDiv] = []
    
    if (ele.match(/[A-Z]{3}/)) {
      let team = {teamID: ele, w: Number(data[i + 1]), l: Number(data[i + 2])}
      records[currentLge][currentDiv].push(team)
    }
  }
